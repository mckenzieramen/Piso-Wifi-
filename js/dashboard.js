...
const updateClientAccount = httpsCallable(functions, "updateClientAccount");
...
function openUnitModal(id=null){
  ...
  openModal(id?"Edit Client / Unit":"Add New Client",`
    <div class="form-grid">
      <div class="field"><label>Client ID *</label><input id="fClientCode" value="${esc(u?.clientCode||"")}" placeholder="CID-0001" disabled><small class="hint">Automatically generated. Client IDs are never reused.</small></div>
      <div class="field"><label>Unit Code *</label><div class="unit-combobox"><input id="fCodeSearch" value="${esc(currentCode)}" placeholder="Search or select 1–50" autocomplete="off" aria-autocomplete="list"><input id="fCode" type="hidden" value="${esc(currentCode)}"><div id="unitCodeOptions" class="unit-options">${codeOptions}</div></div><small class="hint">An active Unit Code can belong to only one customer.</small></div>
      ...
      <div class="field full"><label>Registered Gmail *</label><input id="fEmail" type="email" value="${esc(u?.email||"")}" placeholder="customer@gmail.com" autocomplete="off"><small class="hint">Required. One Gmail can belong to only one customer account.</small></div>
      ...
      <div class="field"><label>Username</label><input id="fUsername" value="${esc(u?.username||"")}" placeholder="Generated automatically" disabled><small class="hint">Full login username: First Name + Client ID, e.g. CliffCID-023.</small></div>
      <div class="field full"><label>Notes</label><textarea id="fNotes" rows="3">${esc(u?.notes||"")}</textarea></div>
      ${id?'<div class="field full"><small class="hint"><b>Temporary password:</b> Client ID. The customer must change it after first login. Admin will never receive the customer’s final password.</small></div>':''}
      ...
      if(!unitCode||!firstName||!lastName||!email) throw new Error("Unit Code, First Name, Last Name and Registered Gmail are required.");
      ...
      if(id){
        const result=await updateClientAccount({unitId:id,...payload});
        const username=result?.data?.username||`${sanitizeUsernamePart(firstName)}${u?.clientCode||""}`;
        await logActivity("Clients",`Edited ${unitCode} — ${firstName} ${lastName} — ${username}`,id);
        await addNotification("client","Client profile updated.",`${firstName} ${lastName} (${u?.clientCode||""}) was updated.`,id);
      }else{
        if(payload.active!==true) throw new Error("A new customer account must be created as Active.");
        const result=await createClientAccount(payload);
        const created=result?.data;
        if(!created?.ok) throw new Error("CLIENT ACCOUNT CREATION FAILED");
        await logActivity("Clients",`Added client account ${created.clientCode} — ${created.username} — ${firstName} ${lastName}`,created.unitId);
        await addNotification("client","New client account created.",`${firstName} ${lastName} (${created.clientCode}) can log in using ${created.username}.`,created.unitId);
        notify(`Created ${created.clientCode}. Username: ${created.username} · Temporary password: ${created.temporaryPassword}`);
      }
    });
